...
public class Icecream implements IIcecream  {
	...
	private LinkedList<Icecream> list = new LinkedList<>(); //메뉴 리스트
	...
	//메서드
	@Override
	public void addIcecream(Icecream i) {           //아이스크림 메뉴 추가
		if(list.contains(i)) {
			System.out.println("이미 있는 메뉴 입니다.");
		}else {
			list.add(i);
			System.out.println(i.name + "을/를 메뉴에 추가했습니다.");
		}
	}
	...
	public int cupSize() {                          //컵 사이즈 선택
		...
		int number = 0;
		es :while(true) {	
			System.out.println("사이즈 메뉴: 파인트 | 쿼터 | 패밀리 | 하프갤런 | 종료");
